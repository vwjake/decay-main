...
func TestAdminEventEditSignups(t *testing.T) {
	...
	if err := AdminEventEdit(EventDetailData{Event: ev, Signups: signups}, me).Render(context.Background(), io.Discard); err != nil {
		t.Fatalf("AdminEventEdit with signups: %v", err)
	}
	if err := AdminEventEdit(EventDetailData{Event: ev}, me).Render(context.Background(), io.Discard); err != nil {
